...
public class FuncoesData {
    ...
    /**
     * Método utilizado para transformar filtro de uma data passada como
     * parâmetro entre 00:00hs de 1o. de janeiro e 23:59:59hs de 31 de dezembro 
     * do ano.
     *
     ...
     *
     */
    public static void to_char(Filtro filtro, String campo, int ano, String tipo)
    {        
        Calendar calendar = getCalendar(new java.util.Date());
        calendar.set(Calendar.YEAR, ano);
        to_char(filtro, campo, 
            calendar.getMinimum(Calendar.DAY_OF_MONTH), Calendar.JANUARY, 
            calendar.getMaximum(Calendar.DAY_OF_MONTH), Calendar.DECEMBER,
            ano, tipo);
    }
    
    /**
     * Método utilizado para transformar filtro de uma data passada como
     * parâmetro entre 00:00hs e 23:59:59hs do dia do mês do ano.
     *
     * @param filtro Filtroda consulta.
     * @param campo Campo que será adicionado na consulta.
     * @param data Data utilizada para a consulta.
     *
     */
    public static void to_char(Filtro filtro, String campo, int dia, int mes, 
                               int ano, String tipo)
    {        
        to_char(filtro, campo, 
            dia, mes, 
    ...
    public static void to_char(Filtro filtro, String campo, Date data, String tipo)
    {        
        Calendar calendar = getCalendar(data);
        to_char(filtro, campo, 
            calendar.get(Calendar.DAY_OF_MONTH), calendar.get(Calendar.MONTH),
            calendar.get(Calendar.DAY_OF_MONTH), calendar.get(Calendar.MONTH),
            calendar.get(Calendar.YEAR), tipo);
    }

    
    /**
     * Método utilizado para transformar filtro de uma data passada como
     * parâmetro entre os dias do ano
     *
     * @param filtro Filtroda consulta.
     * @param campo Campo que será adicionado na consulta.
     * @param data Data utilizada para a consulta.
     *
     */
    public static void to_char(Filtro filtro, String campo, 
                               int diaInicial, int mesInicial, 
                               int diaFinal, int mesFinal,
                               int ano, String tipo)
    {
        Date[] datas = to_char(diaInicial, mesInicial, diaFinal, mesFinal, ano);

        filtro.setTipoCondicaoDate(tipo);

    	filtro.greaterOrEqualsThan(campo, datas[0]);
    	filtro.lessOrEqualsThan(campo, datas[1]);
    }

    /**
     * Método utilizado para gerar um filtro de datas contendo os valores que
    ...
    public static void to_char(Filtro filtro, String campo, 
                               ...
                               int diaFinal, int mesFinal,
                               int anoFinal, String tipo)
    {
        Date[] datas = to_char(diaInicial, mesInicial, anoInicial, diaFinal, mesFinal, anoFinal);
    ...
    public static void adicionarDataInicialEFinalAoFiltro(Filtro filtro, String nomeColuna, Date dataInicial, 
            Date dataFinal, String tipoCondicao)
    {
        Calendar calendar = getCalendar(dataInicial);
        int diaInicial = calendar.get(Calendar.DAY_OF_MONTH);
        int mesInicial = calendar.get(Calendar.MONTH);
        int anoInicial = calendar.get(Calendar.YEAR);

        calendar = FuncoesData.getCalendar(dataFinal);
        ...
        to_char(filtro, nomeColuna, diaInicial, mesInicial, anoInicial, 
                diaFinal, mesFinal, anoFinal, tipoCondicao);
        
    }
    
    public static void adicionarDataInicialEFinalAoFiltro(Filtro filtro, String nomeColuna, String dataInicial, 
            String dataFinal, String tipoCondicao) throws ParseException
    {
        adicionarDataInicialEFinalAoFiltro(filtro, nomeColuna, sdfData.parse(dataInicial), 
            sdfData.parse(dataFinal), tipoCondicao);
    }
    ...
    public static void adicionarFiltroIntervaloDatas(Filtro filtro, 
        String strDataInicio, String strDataFim, String nomeColunaData)
    throws ParseException
    {
        Date dataInicioFiltro = FuncoesData.sdfData.parse(strDataInicio);
        Date dataFimFiltro = FuncoesData.sdfData.parse(strDataFim);

        dataInicioFiltro = getDataComHoraMinima(dataInicioFiltro);
        dataFimFiltro = getDataComHoraMaxima(dataFimFiltro);

        filtro.setTipoCondicaoDate(Condicao.DATETIME);
    	filtro.greaterOrEqualsThan(nomeColunaData, dataInicioFiltro);
    	filtro.lessOrEqualsThan(nomeColunaData, dataFimFiltro);
    }
	
	
	public static void adicionarFiltroIntevaloMesDaData(Filtro filtro, 
			Date data, String nomeColunaData)	
	{
		try
		{
			// SETAR A DATA INICIAL PARA O DIA 1 DO MES ATUAL COM HORA ZERO
			Calendar calInicial = getCalendar(data);
			calInicial.set(Calendar.DAY_OF_MONTH, calInicial.getMinimum(Calendar.DAY_OF_MONTH));
			calInicial = getCalendar(tirarHoras(calInicial.getTime()));
			
			// SETAR A DATA FINAL PARA O DIA 1 DO MES SEGUINTE COM HORA ZERO TB.
			Calendar calFinal = getCalendar(calInicial.getTime());
			calFinal.add(Calendar.MONTH, 1);
			
			adicionarFiltroIntervaloDatas(filtro, 
					formatarDataComHora(calInicial.getTime()),
					formatarDataComHora(calFinal.getTime()), 
					nomeColunaData);
		}
		catch (ParseException erro)
		{
			// NUNCA VAI ACONTECER POIS EU JA FAÇO O FORMATAR PARA DEPOIS FAZER O PARSE
...
//		try 
//		{
//			Date dataIni = getData("21/02/2009");
